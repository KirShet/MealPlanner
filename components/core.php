...
$mysqli =  $dbh = new PDO("mysql:dbname=meal_planner;host=localhost:3306","root","root");
// unset($_SESSION);
...
function RuName($name) {
    return 
    ($name == 'break') ? '<h2>Завтрак</h2>' : 
    (($name == 'snacks') ? '<h2>Перекус</h2>' : 
        (($name == 'lunch') ? '<h2>Обед</h2>' :  
        (($name == 'dinner') ? '<h2>Ужин</h2>' : 
        (($name == 'second_break') ? '<h2>Второй завтрак</h2>' : null))));
}
function Access(){
...
function nutrWeight($eating, $nutr){
    $nutr = 
    $nutr=='carb' ? 'carb_per_100_grams':
    ($nutr=='fats' ? 'fats_per_100_grams':
    ($nutr=='prot' ? 'prot_per_100_grams': false));
